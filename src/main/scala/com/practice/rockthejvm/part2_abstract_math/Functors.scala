...
//noinspection TypeAnnotation,ScalaUnusedSymbol,ScalaWeakerAccess
object Functors {
  ...
  // FUNCTORS generalize a transformation

  // generalizing an API
  def do10xList(list: List[Int]): List[Int]        = list.map(_ * 10)
  def do10Option(option: Option[Int]): Option[Int] = option.map(_ * 10)
